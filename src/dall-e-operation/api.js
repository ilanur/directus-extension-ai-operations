...
export default defineOperationApi({
    ...
    handler: async (
        { operation ="generation", text, image, mask, api_key, amount=1, size='1024x1024', save_assets=true, folderId=null }, 
        ...
        for (let i = 0; i < amount; i++) {
            ...
            result.push(url);
            if (save_assets) {
                //asset data has folder = folder id
                let asset_data = {};
                if(folderId){
                    asset_data = {
                        folder: folderId
                ...
                const id = await files.importOne(url, asset_data); 
                ids.push(id);
            }
        }
